...
> 最后更新：2026-08-18（feature01 需求侧与 UX 全部收口：Q1–Q8 + R1–R3 拍板，过 mainline §6 阶段门 2，下一步阶段门 3 技术方案）
...
**第一阶段（看板 + 多视图）已完工交付。feature01「项目时间管理 + 仪表盘」需求侧与 UX 已全部收口（2026-08-18：UX 问卷 Q1–Q8 + 样张遗留 R1–R3 共 11 项一次性拍板），过 mainline §6 阶段门 2；尚未动正式代码或数据库。** 当前主线 = 进阶段门 3（技术方案、数据与 API 契约，议题清单见 feature01 `STATE.md` §7）。feature01 冷启动入口：`feature-01-项目时间管理-仪表盘/STATE.md`。
...
- **当前阶段**：需求侧与 UX 原型已全部收口（2026-08-18 过 mainline §6 阶段门 2）；下一步进阶段门 3（技术方案、数据与 API 契约）；未动正式代码或数据库。
- **当前模型**：固定六阶段；一行一个节点；一个日期字段；主线/并行两轨各自计算一列间隔、各自顺延；状态三态（未开始/进行中/已完成）；仪表盘行模型 = 每项目默认主线/并行两行 + 展开按钮拆重叠阶段（未展开仅重叠段上下分半）；拖拽 = 节点右键四项菜单双模式（拖拽/拖拽顺延/已完成/未完成）、无默认档、草稿批次统一提交；视觉 = 方向 B 深色 + v3.2 六色 + ) 形分界 + 白圈节点；权限 = workspace admin 全改 / member 仅改自己创建的项目，查看对所有注册成员开放。
- **下一步**：进阶段门 3——数据模型落位、批量更新 API、schema 迁移与备份、Excel 解析器契约、深色主题范围、链式 vs 跨式画法对照、右键菜单 API 形态（议题清单见 feature01 `STATE.md` §7）。
