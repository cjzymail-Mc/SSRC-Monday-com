...
> 冻结日期：2026-08-25
>
> 状态：生产施工、隔离自动验收及用户单独授权的一次性真实运维已完成，进入 `ARCHIVE_VIEW_WAIT_HUMAN`；尚未通过人工体验终验、发布或上线。
>
> 权威来源：用户确认归档基线，并拍板 Q1–Q8 = A / A / A / A / A / B / A / A；密集节点选择方案 B；今日左侧白色蒙版最初确认为 80%，2026-08-25 最新覆盖为 70%；恢复【全项目仪表盘】全屏按钮；单双项目日历按像素密度自适应且不得重叠，今天轴须覆盖到日历顶部。
>
> **2026-08-26 Gate 5 体验覆盖**：归档操作入口仅保留在【单项目仪表盘】项目菜单；【全项目仪表盘】行菜单与【时间表编辑器】均不渲染归档入口。单双仪表盘的【撤销/放弃/更新】操作条只有形成当前本地草稿或存在当前会话可撤销提交后才生成；全项目提交后的撤销态默认收起，仅项目列 hover/focus 时显示红色【撤销】。仅撤销态项目列须提供条件化键盘入口，下一次 Tab 可到达撤销按钮。
>
...
8. 归档入口仅位于单项目仪表盘的项目菜单；全项目行与时间表编辑器不得出现该入口。执行前只出现一次确认弹窗。
9. 归档列表按 `archived_at DESC, name ASC, project_id ASC` 排序；【已归档项目】拥有独立日期范围。原独立视角记忆设计已被 2026-08-25 临时覆盖停用，现行行为见 §5.2/§5.5。
...
- 仅单项目仪表盘的 active 项目菜单显示【归档项目】；全项目 active 行和时间表编辑器均不渲染该动作，归档项目也不显示该动作。
...
- 浏览器视角键原设计为 `workspace × account × context=archived`。**2026-08-25 最新临时覆盖**：单双项目一律暂停读取与写入缩放/中心点/滚动位置记忆；切换标签、页面或项目时恢复“适配全部”的 100% 完整范围。当前页面内仍可临时缩放和拖拽。
- 归档/取消归档导致集合变化时重新钳制已有中心点；无归档项目时展示空状态，不借用 active 范围制造假数据。

### 5.3 今日蒙版

- 两类仪表盘中，若项目满足 `first_node_date <= today <= last_node_date`，则【今天】左侧整体覆盖 70% 不透明度的白色蒙版；今天右侧维持六阶段原色。
- 蒙版覆盖阶段条、节点和背景，但红色今天线置于蒙版与日历的年/季度/月/日层级之上，并从日历顶部贯穿项目行；过去/未来项目不加蒙版。今天仍按服务端 `Asia/Shanghai` 日期语义计算。
...
- 2026-08-25 临时覆盖：进入任一单项目或全项目页面、切换项目、切换任何普通/虚拟标签时，视窗天数重置为当前上下文 `fullDays`、中心点重置为完整范围中心、滚动位置归零；全屏内切标签同样重置，但不得退出全屏。

### 5.6 日历标签防碰撞（2026-08-25 最新覆盖）

- 单项目与全项目共用按 `可用画布宽度 / 当前视窗天数` 计算的日历密度；缩放、容器尺寸变化和全屏切换后都必须重新计算。
- 每日像素宽度 `>=30px` 时显示“日号＋中文星期”，`>=14px 且 <30px` 时只显示日号，`<14px` 时按至少约 30px 标签间距抽样并显示“月/日”。
- 每个日期格使用明确、受边界约束的宽度，标签在本格内裁切，不得侵入相邻日期格；不得在空间不足时继续并排渲染“汉字＋数字”。
- 此规则只改变标尺显示密度，不改变时间范围、节点日期、缩放下限、拖拽命中区或服务端数据。
- 2026-08-25 后续界面覆盖：单双项目统一使用彩色年/月/日三层标尺；季度层与全项目画布上方“完整数据 / 当前可见范围 / 操作提示”整栏移除，且不得保留空白占位。
...
- 单项目仪表盘归档入口经过一次确认；取消零请求，确认后 active 入口消失、项目进入两个归档展示页且只读态正确，取消归档后原标签/顺序恢复；全项目 active 行与时间表编辑器始终没有归档入口。
- 单双仪表盘在既无草稿也无当前会话撤销时不渲染【撤销/放弃/更新】；单纯 hover 不能生成操作条。形成日期/状态本地草稿后操作条主动出现，放弃草稿后消失；全项目提交后的当前会话一次撤销默认隐藏，项目列 hover/focus 时显示且【撤销】为红色；仅撤销态项目列可聚焦，下一次 Tab 进入对应撤销按钮，失焦后收起。
- 全项目项目列/表头内滚轮只纵向滚动项目且不得改变时间视窗；右侧日期/轨道画布内滚轮继续按既有合同缩放。
- 归档列表按时间倒序，归档与 active 使用各自独立日期范围；当前视角记忆停用，切换上下文须各自恢复完整范围。
- 70% 今日蒙版、红色今天线从日历顶部贯穿且层级高于日历、右侧原色在两类仪表盘可见。
- 单项目与全项目在各缩放比例下均无日期标签重叠；空间不足时按 5.6 自动降级或抽样，放大后自动恢复更详细标签。
- 全项目节点在 `<18px` 时变 5px 中性实心点，放大到不重叠时恢复白圈；同日保持聚合。
- 全项目全屏按钮可见且可退出；单项目滚轮锚点缩放、仅横向拖拽、纵向位移为零。
- 单双项目首次进入及切换标签/页面/项目后 `viewportDays == fullDays`；本地存储不得新增 `flowboard:timeline-viewport:*`，当前页临时缩放仍生效。
...
- 真实库/体验库首次批量归档曾是独立人工门，现已在用户 2026-08-25 单独授权后按 §10 完成；该授权不延伸到未来再次批量归档或自动归档。

## 9. 施工与验证记录（2026-08-25～2026-08-26）
...
- UI：`app.js`、`timeline-ui.js`、`timeline.css` 落地单项目归档菜单、首页轻量归档页、全项目归档只读画布、独立归档视角、70% 今日蒙版、今天轴覆盖日历、单双项目自适应无重叠日期标尺、5px 密集点、单项目横向缩放/拖拽与可见全屏入口；2026-08-26 按人工反馈移除全项目行/编辑器归档入口，并把单双仪表盘操作条改为草稿/当前会话撤销驱动。
...
- 2026-08-26 体验覆盖证据：`tests/timeline_ui.test.js` 全绿；Chromium 单项分别验证侧栏按钮固定视口中线、归档只能从单项目发起、单双仪表盘 hover 不显示操作条且形成草稿后才显示，均为 1/1 OK。
- 2026-08-26 后续交互证据：Chromium 专项验证全项目提交后自动收起、项目列 hover 显示红色撤销，以及项目列滚轮纵向滚动且 `viewportDays` 不变，分别 1/1 OK。
- 2026-08-26 AUD-01/02 证据：`test_timeline_portfolio_e_d4_p1_canvas` 覆盖普通左/中/右锚点及全屏连续三档缩放，指针日期漂移均 `<=1 天`；`test_timeline_dashboards_share_context_controller` 覆盖仅撤销态项目列聚焦、下一 Tab 命中红色撤销及失焦收起，分别 1/1 OK。
- 施工收口时的真实库保护：当时仅用 SQLite `mode=ro` 复核 `flowboard.db`，结果为 `user_version=15`、`integrity=ok`、`foreign_key_check=0`、`timeline_projects.archived_at` 不存在；当时未执行 v18 迁移或 bootstrap apply。其后用户新增的真实执行授权及结果见 §10。

## 10. 一次性真实执行记录（2026-08-25，用户新增授权）

- 用户明确指令：“直接归档，仅留下活跃项目”。本次授权只覆盖当前一次性初始化，不改变“未来只手工归档、永不自动归档”的永久规则。
- 根 `flowboard.db`：先生成并验证正式备份包 `backups/flowboard-manual-20260825-062112-330235`（schema v15），并保留迁移前数据库 `backups/flowboard-pre-v18-20260825-142132-031738.db`；随后迁移至 v18。该根库实际包含 0 个 timeline 项目，因此没有归档候选。
- 当前测试数据定位于 `%TEMP%\flowboard-gate5-trial\flowboard-trial.db`：共 99 个项目、872 个节点，schema v18。执行前生成并验证备份包 `%TEMP%\flowboard-gate5-trial\backups\flowboard-manual-20260825-062248-062544`。
- 只读预览以 `today=2026-08-25` 得到 74 个候选，snapshot SHA-256 为 `ccf62d2070427d646d34566983c0f2898c17acfbb86231cb440f3a205e684614`；随后使用完全相同快照在一个事务中 apply。
- 后验：`25 active + 74 archived = 99`；active 中不存在最后节点早于今天的项目，archived 中不存在空项目或最后节点等于/晚于今天的项目；`integrity_check=ok`、`foreign_key_check=0`、bootstrap 完成审计 1 条、项目归档审计 74 条。
- 此前施工、验证与运维记录已由用户分批提交至 `d1d9b75`；未 push、发布或部署。本次治理、真源蒸馏及 2026-08-26 体验修复尚未提交。
