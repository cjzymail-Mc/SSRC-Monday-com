...
class CollaborationTests(unittest.TestCase):
    ...
    def test_v12_to_v14_subscription_merge_backup_restore_and_idempotence(self):
        ...
        backup=migrate(db_path);self.assertIsNotNone(backup);self.assertTrue(Path(backup).name.startswith("v12-pre-v15-"))
        ...
        backup_count=len(list((root/"backups").glob("v12-pre-v15-*.db")));self.assertIsNone(migrate(db_path));self.assertEqual(len(list((root/"backups").glob("v12-pre-v15-*.db"))),backup_count)
