...
class CollaborationE2E(unittest.TestCase):
    ...
    def login(self,user,viewport):
        context=self.browser.new_context(viewport=viewport);page=context.new_page();page.goto(self.base);page.locator("#loginUser").fill(user);page.locator("#loginPassword").fill("test-password");page.locator("#loginForm button[type=submit]").click();page.locator(".task-row").first.wait_for();return context,page
