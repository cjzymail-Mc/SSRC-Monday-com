...
class I12E2E(unittest.TestCase):
    ...
    def login(self,user,width=1280):
        context=self.browser.new_context(viewport={"width":width,"height":844});page=context.new_page();page.goto(self.base);page.locator("#loginUser").fill(user);page.locator("#loginPassword").fill("test-password");page.locator("#loginForm button").click();reveal_legacy_board(page);return context,page
    def test_two_context_realtime_fallback_notifications_search_audit_and_mobile(self):
        ...
        member.evaluate("showNotifications()");task_notices=member.locator('#listModal.open .notification-item[data-task="1"]');mention=task_notices.filter(has=member.locator("span").filter(has_text=re.compile(r"^mention\.added · ")));changed=task_notices.filter(has=member.locator("span").filter(has_text=re.compile(r"^task\.changed · ")));mention.first.wait_for();changed.first.wait_for();self.assertEqual(mention.count(),1);self.assertGreaterEqual(changed.count(),1);self.assertIn("Realtime Needle",mention.first.text_content());self.assertIn("Realtime Needle",changed.first.text_content());member.locator("#readAllNotifications").click();member.wait_for_function("document.querySelectorAll('#listModal.open .notification-item.unread').length === 0")
        admin.once("dialog",lambda dialog:dialog.accept("Needle"));admin.evaluate("showGlobalSearch()");admin.locator("#listModal.open").wait_for();self.assertEqual({admin.locator(".search-result b").nth(i).text_content().lower() for i in range(admin.locator(".search-result b").count())},{"board","task","comment","attachment","member"});admin.locator("#listModal [data-close]").click()
        admin.evaluate("showAudit()");admin.locator("#listModal.open",has_text="task.field_values.updated").wait_for();self.assertTrue(admin.locator("a[href$='audit.csv']").is_visible());self.assertEqual(member.evaluate("fetch('/api/admin/workspaces/1/audit').then(r=>r.status)"),403)
        member_context.route("**/events/stream*",lambda route:route.abort());reload_legacy_board(member);member.wait_for_function("document.querySelector('#realtimeState').textContent.includes('轮询降级')",timeout=10000);admin.evaluate("api('/api/tasks/1').then(t=>api('/api/tasks/1','PATCH',{version:t.version,title:'Fallback Needle'}))");member.locator(".task-row",has_text="Fallback Needle").wait_for(timeout=12000)
        member_context.unroute("**/events/stream*");reload_legacy_board(member);member.wait_for_function("document.querySelector('#realtimeState').textContent.includes('实时')",timeout=10000)
        mobile_context,mobile=self.login("u1",390);self.assertEqual(mobile.evaluate("document.documentElement.scrollWidth <= innerWidth"),True);mobile.evaluate("showNotifications()");modal=mobile.locator("#listModal.open .task-modal");modal.wait_for();box=modal.bounding_box();self.assertIsNotNone(box);self.assertLessEqual(box["width"],390)
