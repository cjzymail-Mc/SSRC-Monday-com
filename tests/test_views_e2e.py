...
from legacy_board_e2e import goto_legacy_board, reload_legacy_board, reveal_legacy_board
...
class FlowboardViewsE2E(unittest.TestCase):
    ...
    def test_real_app_tabs_saved_resize_drag_calendar_viewer_and_mobile(self):
        ...
        reload_legacy_board(page); page.locator(".dynamic-grid").first.wait_for(); after = page.locator(".dynamic-grid").first.get_attribute("style")
    ...
    def test_real_app_hierarchy_dependency_cycle_progress_and_date_policy(self):
        ...
        reload_legacy_board(page); page.locator(f'[data-task="{parent_id}"]').click(); page.locator("#addDependencyBtn").wait_for()
        ...
        self.assertEqual(complete_status,200);page.locator('[data-close="taskModal"]').evaluate("el => el.click()");reload_legacy_board(page);page.locator(f'[data-task="{parent_id}"]').click();page.get_by_text("当前不阻塞").wait_for();self.assertIn("2",page.locator(".relation-section").first.text_content())
        page.locator('[data-close="taskModal"]').evaluate("el => el.click()")
    ...
    def test_real_app_cross_board_mirror_formula_and_private_acl(self):
        ...
        reload_legacy_board(page);page.locator(f'[data-task="{setup["source"]}"]').wait_for();row=page.locator(f'[data-task="{setup["source"]}"]');self.assertIn("7",row.text_content());self.assertIn("14",row.text_content())
        ...
        reload_legacy_board(page);row=page.locator(f'[data-task="{setup["source"]}"]');row.wait_for();self.assertIn("9",row.text_content());self.assertIn("18",row.text_content())
    ...
    def test_real_app_timeline_gantt_and_mobile_fallback(self):
        ...
        goto_legacy_board(page,f'{self.base}/?board=1&view={setup["timeline"]}','.schedule-bar');self.assertEqual(page.locator('.schedule-bar').count(),3);self.assertEqual(page.locator('#scheduleSource').input_value(),str(setup["field"]));self.assertEqual(page.locator('#scheduleScale').input_value(),'week');self.assertIn(f'view={setup["timeline"]}',page.url)
        page.locator('#scheduleScale').select_option('month');page.locator('.schedule-bar').first.wait_for();self.assertEqual(page.locator('#scheduleScale').input_value(),'month');reload_legacy_board(page,'.schedule-bar');self.assertEqual(page.locator('#scheduleScale').input_value(),'month')
        ...
        page.once('dialog',lambda dialog:dialog.accept('set-default'));page.locator('#viewBtn').click();page.wait_for_timeout(500);goto_legacy_board(page,f'{self.base}/?board=1','.schedule-bar');self.assertEqual(page.locator('#scheduleScale').input_value(),'month')
        ...
        bar_selector=f'[data-schedule-bar="{setup["tasks"][0]}"]';goto_legacy_board(page,f'{self.base}/?board=1&view={setup["gantt"]}',bar_selector);bar=page.locator(bar_selector);box=bar.bounding_box();page.once('dialog',lambda dialog:dialog.dismiss());page.mouse.move(box['x']+box['width']/2,box['y']+box['height']/2);page.mouse.down();page.mouse.move(box['x']+box['width']/2+12,box['y']+box['height']/2);page.mouse.up();page.locator('#toast',has_text='排期已更新').wait_for();reload_legacy_board(page,bar_selector);bar=page.locator(bar_selector);self.assertIn('2026-08-02',bar.get_attribute('data-start'))
        box=bar.bounding_box();page.once('dialog',lambda dialog:dialog.dismiss());page.mouse.move(box['x']+box['width']-2,box['y']+box['height']/2);page.mouse.down();page.mouse.move(box['x']+box['width']+10,box['y']+box['height']/2);page.mouse.up();page.locator('#toast',has_text='排期已更新').wait_for();reload_legacy_board(page,bar_selector);self.assertEqual(page.locator(bar_selector).get_attribute('data-start'),'2026-08-02');self.assertTrue(page.locator('.schedule-links path').count());self.assertTrue(page.locator('.schedule-bar.critical').count()>=2);self.assertEqual(page.locator('.schedule-bar.milestone').count(),1)
        goto_legacy_board(page,f'{self.base}/?board=1&view={setup["filtered"]}','.schedule-bar');self.assertEqual(page.locator('.schedule-bar').count(),1);self.assertEqual(page.locator('.schedule-links path').count(),0);goto_legacy_board(page,f'{self.base}/?board=1&view={setup["gantt"]}','.schedule-bar')
        ...
        mobile_context,mobile=self.login_page('u2',viewport={"width":390,"height":844});goto_legacy_board(mobile,f'{self.base}/?board=1&view={setup["timeline"]}','.schedule-bar');self.assertEqual(mobile.locator('[data-schedule-edit]').count(),0);denied=mobile.evaluate(f"""async()=>{{const s=await(await fetch('/api/session')).json(),b=await(await fetch('/api/bootstrap')).json(),t=await(await fetch('/api/tasks/{setup['tasks'][1]}')).json();return(await fetch('/api/tasks/{setup['tasks'][1]}/schedule',{{method:'POST',headers:{{'Content-Type':'application/json','X-CSRF-Token':s.csrf_token}},body:JSON.stringify({{version:t.version,board_version:b.board.version,source:{{kind:'dynamic',id:{setup['field']}}},start:'2026-09-01',end:'2026-09-02'}})}})).status}}""");self.assertEqual(denied,403);mobile_context.close()
        owner_context,owner=self.login_page(viewport={"width":390,"height":844});edit_selector=f'[data-schedule-edit="{setup["tasks"][1]}"]';goto_legacy_board(owner,f'{self.base}/?board=1&view={setup["timeline"]}',edit_selector);edit=owner.locator(edit_selector);answers=iter(['2026-09-01','2026-09-04',False]);owner.on('dialog',lambda dialog:dialog.accept(next(answers)) if dialog.type=='prompt' else dialog.dismiss());edit.click();owner.locator('#toast',has_text='排期已更新').wait_for();reload_legacy_board(owner,f'[data-schedule-bar="{setup["tasks"][1]}"][data-start="2026-09-01"]');owner_context.close()
    ...
    def test_real_app_chart_views_and_dashboard_widgets(self):
        ...
        for kind,view_id in setup['views'].items():
            goto_legacy_board(page,f'{self.base}/?board=1&view={view_id}','.chart-data');self.assertTrue(page.locator('.chart-data tbody tr').count());self.assertEqual(page.locator('#chartType').input_value(),kind)
