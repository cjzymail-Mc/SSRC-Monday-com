...
const denseFocused=Timeline.renderAll([denseProject],{today:'2026-07-01',viewport:{center:'2026-08-01',days:61},viewportWidth:760,focusedProjectId:7});
...
ok(!denseFocused.includes('timeline-node-cluster')&&(denseFocused.match(/class="timeline-node-flag"/g)||[]).length===2&&denseFocused.includes('data-node-flag-key="7:71"')&&denseFocused.includes('data-node-flag-key="7:72"'),'focused dense row keeps individual nodes and one-to-one flags so collision avoidance can preserve exact hover ownership');
...
ok(html.includes('is-single-profile')&&html.includes('has-node-flags')&&html.includes('data-node-flag-visible="true"')&&html.includes('class="timeline-node-flag"'),'single dashboard is permanently expanded and renders node flags by default');
...
const weeklyFocused=Timeline.renderAll([weeklyActive],{timeScope:'week',today:'2026-08-04',focusedProjectId:31});
ok(weeklyFocused.includes('is-focused has-node-flags')&&weeklyFocused.includes('data-node-flag-key="31:312"')&&weeklyFocused.includes('data-node-flag-key="31:313"'),'weekly dashboard uses the shared expanded-row flag behavior');
...
const monthlyFocused=Timeline.renderAll([monthlyActive],{timeScope:'month',today:'2026-08-04',focusedProjectId:33});
ok(monthlyFocused.includes('is-focused has-node-flags')&&monthlyFocused.includes('data-node-flag-key="33:331"'),'monthly dashboard uses the shared expanded-row flag behavior');
